...
class SingleTrackSolution:
    ...
    @staticmethod
    def found(
        cost: float, max_delay: float, used_links: list[tuple[str, str]]
    ) -> "SingleTrackSolution":
...
def multicast_heuristic(network: nx.DiGraph, track: Track) -> SingleTrackSolution:
    ...
    def augment(node: str):
        ...
        for tree_node in set(tree.nodes) - loop_causing_nodes:
            ...
            new_base_e2e_delay = (
                latencies[node] + network.get_edge_data(*replacement_edge)["latency"]
            )
            old_base_e2e_delay = (
                latencies[previous_node]
                + network.get_edge_data(*to_be_replaced_edge)["latency"]
            )
            delay_balance = new_base_e2e_delay - old_base_e2e_delay

            cost_balance = (
                network.get_edge_data(*replacement_edge)["cost"]
                - network.get_edge_data(*to_be_replaced_edge)["cost"]
            )

            subtree = subtree_in_tree(tree_node)

            # If the replacement comes with cost reductions, and the delay budget is met by redirecting the traffic
            if cost_balance < best_replacement.cost_balance and all(
                latencies[v] + delay_balance <= track.delay_budget for v in subtree
            ):
        ...
        if best_replacement.cost_balance < 0 or (
            best_replacement.cost_balance == 0 and best_replacement.delay_balance < 0
        ):
    ...
    def add_subscriber(node: str) -> str | None:
        ...
        best_edge = min(
            filter(
                lambda edge, node=node: edge[0] in tree.nodes
                and edge[1] == node
                and latencies[edge[0]] + network.get_edge_data(*edge)["latency"]
                <= track.delay_budget,
                network.edges,
            ),
            key=lambda edge: (
...
def get_optimal_topology_for_a_single_track(
    network: nx.DiGraph, track: Track
) -> SingleTrackSolution:
    ...
    link_usages = lp.LpVariable.dicts("y", network.edges, 0, None, cat=lp.LpContinuous)
    ...
    prob += (
        lp.lpSum(
            [
                data["cost"] * link_usages[(node1, node2)]
    ...
    for stream in track.streams.keys():
        for link in network.edges:
            prob += (
                link_usages[link] >= transmission_bitrates[stream][link],
    ...
    for stream, node_reliabilities in track.streams.items():
        for node in network.nodes.keys():
            ...
            in_going = lp.lpSum(
                [
                    transmission_bitrates[stream][link]
                    for link in network.edges
                    if link[1] == node
                ]
            )
            ...
            prob += (
                in_going - out_going == node_reliabilities[node],
    ...
    for stream in track.streams.keys():
        for link in network.edges:
            prob += (
                selected_links[stream][link] * M >= transmission_bitrates[stream][link],
    ...
    for stream in track.streams.keys():
        prob += (
            lp.lpSum(
                [
                    selected_links[stream][(node1, node2)] * data["latency"]
                    for node1, node2, data in network.edges(data=True)
                ]
            )
            <= track.delay_budget,
    ...
    max_delay = max(
        track.delay_budget + prob.constraints[f"delay_budget_for_{stream}"].value()
        for stream in track.streams.keys()
    )
...
def get_single_track_optimizer(
    type: SingleTrackOptimizerType,
) -> Callable[[nx.DiGraph, Track], SingleTrackSolution]:
...
class MultiTrackSolution:
    ...
    def success(self) -> bool:
        return self.explicit_success and all(
            solution.success for solution in self.solutions.values()
        )
    ...
    def used_links_per_track(self) -> dict[str, list[tuple[str, str]]]:
        if not self.explicit_success:
            return {}
        return {
            track_id: solution.used_links
            for track_id, solution in self.solutions.items()
        }
...
def get_optimal_topology_for_multiple_tracks(
    network: nx.DiGraph, tracks: dict[str, Track]
) -> MultiTrackSolution:
    prob = lp.LpProblem("MoQ_relay_topology_optimization", lp.LpMinimize)
    ...
    for track_id, track in tracks.items():
        transmission_bitrates[track_id] = lp.LpVariable.dicts(
            f"x_{track_id}",
            (track.streams.keys(), network.edges),
            0,
            None,
            cat=lp.LpContinuous,
        )
    ...
    prob += (
        lp.lpSum(
            [
                data["cost"] * link_usages[track_id][(node1, node2)]
                for node1, node2, data in network.edges(data=True)
    ...
    for track_id, track in tracks.items():
        for stream in track.streams.keys():
            for link in network.edges:
                prob += (
                    link_usages[track_id][link]
                    >= transmission_bitrates[track_id][stream][link],
    ...
    for track_id, track in tracks.items():
        for stream in track.streams.keys():
            for link in network.edges:
                prob += (
                    selected_links[track_id][stream][link] * M
                    >= transmission_bitrates[track_id][stream][link],
    ...
    for track_id, track in tracks.items():
        for stream in track.streams.keys():
            prob += (
                lp.lpSum(
                    [
                        selected_links[track_id][stream][(node1, node2)]
                        * data["latency"]
                        for node1, node2, data in network.edges(data=True)
                    ]
                )
    ...
    for track_id, track in tracks.items():
        ...
        max_delay = max(
            track.delay_budget
            + prob.constraints[f"delay_budget_for_{track_id}_{stream}"].value()
            for stream in track.streams.keys()
        )

        solutions[track_id] = SingleTrackSolution.found(
            objective, max_delay, used_links
        )

    return MultiTrackSolution.found(solutions)


def multi_to_single_track_adapter_factory(
    strategy: Callable[[nx.DiGraph, Track], SingleTrackSolution],
) -> Callable[[nx.DiGraph, Track], MultiTrackSolution]:

    def multi_to_single_track_adapter(
        network: nx.DiGraph, tracks: dict[str, Track]
    ) -> MultiTrackSolution:
        solutions = {}
...
def get_multi_track_optimizer(
    type: str, **kwargs
) -> Callable[[nx.DiGraph, dict[str, Track], bool], MultiTrackSolution]:
